...
const tableHasColumn = async (db: Database, table: string, column: string): Promise<boolean> => {
...
export const runMigrations = async (): Promise<void> => {
  ...
  if (!(await tableHasColumn(db, "recipes", "isCustom"))) {
    await db.executeSql("ALTER TABLE recipes ADD COLUMN isCustom INTEGER NOT NULL DEFAULT 0");
  }

  // Migration: ensure meal_plan_entries.forMember exists; if not, recreate the
  // table with the new UNIQUE(date, mealType, forMember) constraint so multiple
  // members can have different meals in the same slot.
  if (!(await tableHasColumn(db, "meal_plan_entries", "forMember"))) {
    await db.transaction(async (tx) => {
      ...
      await tx.executeSql("DROP TABLE meal_plan_entries");
      await tx.executeSql("ALTER TABLE meal_plan_entries_new RENAME TO meal_plan_entries");
    });
  }
};
